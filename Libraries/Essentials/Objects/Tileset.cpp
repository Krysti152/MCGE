...
#include <MCGE/Essentials/Objects/Headers/Tileset.hpp>

gm::Tile::Tile() {
}
gm::Tile::Tile(int _id) {
    id = _id;
}
gm::Tile::~Tile() {
}
// set texture id of a tile
void gm::Tile::setId(int _id) {
    id = _id;
}
...
int gm::Tileset::getTexIdByName(std::string _name) {
...
// set tile's id
void gm::Tileset::setTile(Vector2 _pos, int _id) {
...
void gm::Tileset::setTile(int _x, int _y, int _id) {
    setTile(Vector2{_x,_y}, _id);
}

void gm::Tileset::setTile(int _x, int _y, std::string _tex_name) {
    // std::cout<<_tex_name<<" --> "<<tex_names[_tex_name]<<std::endl;
    setTile(Vector2{_x,_y}, tex_names[_tex_name]);
}

gm::Tile gm::Tileset::getTile(int _x, int _y) {
...
gm::Tile gm::Tileset::getTile(Vector2 _vec) {
    return getTile(_vec.x, _vec.y);
}

// updates information about tiles
void gm::Tileset::update() {
...
void gm::Tileset::fill(Vector2 _a, Vector2 _b, int _id) {
...
// load textures
void gm::Tileset::loadTiles(std::string _path = "assets/sprites/tileset/", std::string _new_tex_name = "TILE_") {
...
void gm::Tileset::loadMap(std::string _path = "assets/tilesets/test.tileset") {
...
void gm::Tileset::saveTileset(std::string _path = "assets/tilesets/test.tileset") {
...
void gm::Tileset::renderTiles() {
    for ( const auto &[key, value]: tiles ) {
        WINDOW.draw(tiles[key].sprite);
    }
}
void gm::Tileset::showTextures() {
